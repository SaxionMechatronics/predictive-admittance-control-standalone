...
class NMPCController:
    ...
    def _setup_ocp(self):
        ...
        terminal_cost = (
            ...
            + 0.5 * ca.transpose(hN[7:] - refs[7:]) @ ca.diag(W[7:]) @ (hN[7:] - refs[7:])
        )
        ...
        self.solver = AcadosOcpSolver(ocp, json_file=f"trajectory_track_mpc.json", 
...
class FiberthexMuJoCoInterface:
    ...
    def run_control_loop(self, reference_fn: Callable[[float], dict],
        ...
        if render:
            ...
            camera_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_CAMERA, "scene")
            if camera_id != -1:
