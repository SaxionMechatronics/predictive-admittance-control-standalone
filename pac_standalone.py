"""
Standalone Example of using Acados to implement Predictive Admittance Control to control Fully Actuated Hexarotor (Fiberthex) simulated in MuJoCo
...
Main data fields:
    ...
    - references: Dict with position(n,3), quaternion(n,4), velocity(n,3), angular_velocity(n,3)
    - actual_states: Dict with position(n,3), quaternion(n,4), velocity(n,3), angular_velocity(n,3),
                     rotor_forces(n,6), impedance_position(n,3), impedance_velocity(n,3), contact_forces(n,3)
    ...
    - controller_config: Complete controller parameters (weights, horizon, impedance, etc.)
    - simulation_config: Simulation parameters (dt, duration, etc.)

For a helper function to inspect log files, see inspect_log_file() at the end of this script.
"""
...
class HexarotorConfig:
    ...
    ee_offset: np.ndarray = field(default_factory=lambda: np.array([0.60188765563, -0.3475, 0.0]))
    ...
    nx: int = 28  # States: [pos(3), quat(4), vel(3), angvel(3), rotor forces(6), imp_pos(3), imp_vel(3), contact_forces(3)]
    ...
    def get_hover_force(self) -> float:
        ...
        hover_force_per_rotor = total_hover_force / self.num_rotors / np.cos(np.deg2rad(self.alpha_tilt))
        return hover_force_per_rotor
...
class ControllerConfig:
    ...
    position_weight: np.ndarray = field(default_factory=lambda: np.array([1e1, 1e1, 1e1]))
    quaternion_weight: float = 1e4
    velocity_weight: np.ndarray = field(default_factory=lambda: np.array([10.0, 10.0, 10.0]))
    angular_velocity_weight: np.ndarray = field(default_factory=lambda: np.array([10.0, 10.0, 10.0]))
    ...
    look_ahead_reference: bool = False  # If True, use different references for each horizon point
...
class HexarotorDynamics:
    ...
    def setup_symbolic_model(self) -> Tuple[ca.SX, ca.SX, ca.SX]:
        ...
        omega_term = R @ (ca.cross(omega_dot,r_ee) + ca.cross(omega, ca.cross(omega,r_ee)))
        
        pos_dot = vel
        # Reference: Eq 2c from [1]
        quat_dot = hamilton_prod(q, ca.vertcat(0, omega)) / 2
        # Reference: Eq 2a from [1]
        vel_dot = ((R @ self.GF @ rotor_thrusts) + contact_forces) / cfg.mass + cfg.gravity + omega_term
        ...
        imp_vel_dot = ca.inv(M_mat) @ (-D_mat @ imp_vel - K_mat @ imp_pos + contact_forces)
        # imp_vel_dot = ca.SX.zeros(3)
        
        contact_forces_dot = ca.SX.zeros(3)
        
        x_dot = ca.vertcat(pos_dot, quat_dot, vel_dot, omega_dot, rotor_thrusts_dot, imp_pos_dot, imp_vel_dot, contact_forces_dot)
...
class NMPCController:
    ...
    def __init__(self, dynamics: HexarotorDynamics,
    ...
    def _setup_pac_objective_function(self, states, controls, xdot, refs, W, W_u):
        ...
        stage_cost = (
            0.5 * ca.transpose(p - imp_p - refs[:3]) @ ca.diag(W[:3]) @ (p - imp_p - refs[:3])
            + 0.5 * dist_quat(q, refs[3:7]) * W[3]
            + 0.5 * ca.transpose(pdot - imp_pdot - refs[7:10]) @ ca.diag(W[7:10]) @ (pdot - imp_pdot - refs[7:10])
            + 0.5 * ca.transpose(omega - refs[10:13]) @ ca.diag(W[10:13]) @ (omega - refs[10:13])
            + 0.5 * ca.transpose(pddot - imp_pddot - refs[13:16]) @ ca.diag(W[13:16]) @ (pddot - imp_pddot - refs[13:16])
            + 0.5 * ca.transpose(omega_dot - refs[16:19]) @ ca.diag(W[16:19]) @ (omega_dot - refs[16:19])
    ...
    def _setup_traj_track_objective_function(self, states, controls, xdot, refs, W, W_u):
        ...
        stage_cost = (
            ...
            + 0.5 * dist_quat(q, refs[3:7]) * W[3]
            + 0.5 * ca.transpose(pdot - refs[7:10]) @ ca.diag(W[7:10]) @ (pdot - refs[7:10])
            + 0.5 * ca.transpose(omega - refs[10:13]) @ ca.diag(W[10:13]) @ (omega - refs[10:13])
            + 0.5 * ca.transpose(pddot - refs[13:16]) @ ca.diag(W[13:16]) @ (pddot - refs[13:16])
            + 0.5 * ca.transpose(omega_dot - refs[16:19]) @ ca.diag(W[16:19]) @ (omega_dot - refs[16:19])
            + 0.5 * ca.transpose(controls) @ ca.diag(W_u) @ controls
        )
    ...
    def _setup_ocp(self):
        ...
        kappa = ca.SX.sym('kappa', 1)   # kappa = 1 when in contact, 0 otherwise
        
        stage_cost, terminal_cost = self._setup_pac_objective_function(states, controls, x_dot, refs, W, W_u)
        ...
        model.p = ca.vertcat(refs, W, W_u, self.dynamics.M_sym, self.dynamics.D_sym, self.dynamics.K_sym, self.dynamics.kappa_sym)
        ...
        self.solver = AcadosOcpSolver(ocp, json_file="pac_ocp.json", 
                                      generate=self.ctrl_cfg.gen_build_code, 
                                      build=self.ctrl_cfg.gen_build_code)
        self.integrator = AcadosSimSolver(ocp, generate=self.ctrl_cfg.gen_build_code, 
                                          build=self.ctrl_cfg.gen_build_code)
        
    def set_ocp_parameters(self, reference: dict, current_time: float = 0.0, reference_fn: Callable[[float], dict] = None, contact_forces: np.array = np.zeros(3)):
        """Set OCP parameters
        
        Args:
            reference: Reference dict at current time
            current_time: Current simulation time (used for look-ahead)
            reference_fn: Reference function that takes time and returns reference dict (required if look_ahead_reference=True)
            contact_forces: Contact forces acting on the end effector (optional, defaults to [0, 0, 0])
        """
        ...
        if self.ctrl_cfg.look_ahead_reference:
            ...
            if reference_fn is None:
                raise ValueError("reference_fn must be provided when look_ahead_reference=True")
    ...
    def compute_control(self, mj_state: np.ndarray, reference: dict, 
                       current_time: float = 0.0, reference_fn: Callable[[float], dict] = None,
        ...
        state = np.concatenate([mj_state, self.impedance_position, self.impedance_velocity, contact_forces])
    ...
    def get_impedance_state(self):
        ...
        # The internal NMPC dt is higher that the control dt_control (e.g. 0.1s > 0.01s)
        # The simulated impedance state is only updated every (dt/dt_control) control steps
        ...
        if self.counter == 10:
            ...
            impedance_position = self.impedance_position
            impedance_velocity = self.impedance_velocity
            self.counter += 1
...
class MuJoCoInterface:
    ...
    def __init__(self, model, controller: NMPCController,
                 robot_config: HexarotorConfig,
    ...
    def _find_actuators(self):
        ...
        actuator_names = ['Force_X', 'Force_Y', 'Force_Z', 
    ...
    def _setup_contact_force_filter(self):
        ...
        self.filter_b, self.filter_a = signal.butter(filter_order, normalized_cutoff, btype='low', analog=False)
        ...
        print(f"Contact force filter initialized: {filter_order}nd order Butterworth, {cutoff_freq} Hz cutoff")

    def _add_reference_sphere_to_scene(self, scene, position: np.ndarray, geom_idx: Optional[int] = None):
    ...
    def initialize_hover(self):
        ...
        for i, key in enumerate(['Force_X', 'Force_Y', 'Force_Z', 'Torque_X', 'Torque_Y', 'Torque_Z']):
    ...
    def run_control_loop(self, reference_fn: Callable[[float], dict],
        ...
        try:
            camera_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_CAMERA, "parallel_cam")
        except:
        ...
        if self.sim_config.render_video:
            ...
            final_video_filename = video_filename if video_filename else self.sim_config.video_filename
            print(f"\nVideo recording enabled: {self.sim_config.video_width}x{self.sim_config.video_height} @ {self.sim_config.video_fps} FPS")
        ...
        print(f"Control decimation: {decimation} (update every {decimation * self.dt:.3f}s)")
        print("=" * 70)
        
        start_time = time.time()
        last_print_time = 0
        control = np.zeros(self.robot_config.num_rotors)
        frame_interval = int(1.0 / (self.sim_config.video_fps * self.dt)) if self.sim_config.render_video else 0
        ...
        try:
            for step in range(num_steps):
                ...
                if step % decimation == 0:
                    ...
                    control = self.controller.compute_control(state, reference, current_time, reference_fn, contact_forces)
                    ...
                    if hasattr(self.controller, 'get_solver_stats'):
                        ...
                        if stats['status'] not in [0, 2, 5]:
                            print(f"\nWarning at t={current_time:.2f}s: Solver status {stats['status']}")

                    # Get predictions if available
                    if hasattr(self.controller, 'get_prediction'):
                        predicted_states, predicted_controls = self.controller.get_prediction()
                ...
                if render and viewer is not None:
                    ...
                    if viewer.user_scn is not None:
                        ...
                        self._add_reference_sphere_to_scene(viewer.user_scn, self._ref_position, geom_idx=0)

                # Capture video frame at specified FPS
                if self.sim_config.render_video and video_renderer is not None and step % frame_interval == 0:
                ...
                self.log_impedance_position.append(self.controller.impedance_position.copy())
                self.log_impedance_velocity.append(self.controller.impedance_velocity.copy())
                ...
                self.log_contact_forces.append(contact_forces if 'contact_forces' in locals() else np.zeros(3))
                ...
                if verbose and (current_time - last_print_time) >= 1.0:
                    ...
                    print(f"t={current_time:5.2f}s | "
                          ...
                          f"ref=[{ref_pos[0]:6.3f}, {ref_pos[1]:6.3f}, {ref_pos[2]:6.3f}] | "
                          f"err={error:6.4f}m")
        ...
        print(f"Total time: {total_time:.2f}s (realtime factor: {duration/total_time:.2f}x)")
        ...
        ref_positions = np.array([ref.get('position', np.zeros(3)) for ref in self.log_references])
        ref_quaternions = np.array([ref.get('quaternion', np.array([1.0, 0.0, 0.0, 0.0])) for ref in self.log_references])
    ...
    def _save_video_frames(self, frames: list, filename: str):
        ...
        if self.sim_config.save_video_frames_as_images:
            ...
            for i, frame in enumerate(frames):
                ...
                if self.sim_config.video_show_progress and (i % 10 == 0 or i == len(frames) - 1):
                    progress = (i + 1) / len(frames) * 100
                    print(f"\r  Progress: {progress:.1f}% ({i + 1}/{len(frames)})", end='', flush=True)
        ...
        try:
            ...
            print(f"  Resolution: {self.sim_config.video_width}x{self.sim_config.video_height}")
    ...
    def save_predictions_to_file(self, filename: str = "predictions_log.pkl"):
        ...
        ref_positions = np.array([ref.get('position', np.zeros(3)) for ref in self.log_references])
        ref_quaternions = np.array([ref.get('quaternion', np.array([1.0, 0.0, 0.0, 0.0])) for ref in self.log_references])
        ref_velocities = np.array([ref.get('velocity', np.zeros(3)) for ref in self.log_references])
        ref_angular_velocities = np.array([ref.get('angular_velocity', np.zeros(3)) for ref in self.log_references])
        ...
        predictions_data = {
            'metadata': {
                'description': 'Predictive Admittance Control (PAC) simulation log file',
                'timestamp': datetime.now().isoformat(),
                'version': '1.0',
                'data_format': {
                    'times': 'Time stamps (n,) [seconds]',
                    'predicted_states': 'List of predicted state trajectories from MPC, each (N+1, nx)',
                    'predicted_controls': 'List of predicted control trajectories from MPC, each (N, nu)',
                    'references': 'Reference trajectories as dict with position(n,3), quaternion(n,4), velocity(n,3), angular_velocity(n,3)',
                    'actual_states': 'Measured states as dict with position(n,3), quaternion(n,4), velocity(n,3), angular_velocity(n,3), rotor_forces(n,6), impedance_position(n,3), impedance_velocity(n,3), contact_forces(n,3)',
        ...
        if filename.endswith('.pkl'):
            ...
            print(f"  Includes metadata, all configurations, and {len(self.log_time)} time steps")
            print(f"  To inspect: use inspect_log_file('{filepath}') or load with pickle.load()")
            ...
            np.savez(filepath, **{
                ...
                'actual_quaternions': predictions_data['actual_states']['quaternion'],
                'actual_velocities': predictions_data['actual_states']['velocity'],
                'actual_angular_velocities': predictions_data['actual_states']['angular_velocity'],
            ...
            print(f"✓ Predictions saved to: {filepath}")
            print(f"  Note: .npz format doesn't include metadata, configs, or predicted trajectories. Use .pkl for full data.")
        else:
...
def constant_reference(position=None, quaternion=None, velocity=None, angular_velocity=None):
...
def trajectory_reference(positions: np.ndarray,
                        quaternions: np.ndarray,
                        times: np.ndarray) -> Callable[[float], dict]:
    """
...
def example_fiberthex_hover():
    ...
    csv_path = logger.log_results(results, experiment_name="hover", reference_trajectory=reference)
...
def example_fiberthex_tracking():
    ...
    print("\nLogging results...")
    logger = ExperimentLogger(robot_config, controller_config, sim_config)
    csv_path = logger.log_results(results, experiment_name="tracking", reference_trajectory=reference)
...
def example_fiberthex_circle():
    ...
    print("\nLogging results...")
    logger = ExperimentLogger(robot_config, controller_config, sim_config)
    csv_path = logger.log_results(results, experiment_name="circle", reference_trajectory=circular_trajectory)
...
def example_fiberthex_interaction():
    ...
    quaternions = np.array([[np.cos(np.deg2rad(15)), 0.0, 0.0, np.sin(np.deg2rad(15))],
    ...
    print("\nLogging results...")
    logger = ExperimentLogger(robot_config, controller_config, sim_config)
    csv_path = logger.log_results(results, experiment_name="APhI", reference_trajectory=reference)
...
def inspect_log_file(filepath: str):
    ...
    try:
        ...
        # Print data summary
        print("DATA SUMMARY:")
        print(f"  Number of time steps: {len(data['times']) if 'times' in data else 'N/A'}")
        print(f"  Duration: {data['times'][-1]:.2f}s" if 'times' in data and len(data['times']) > 0 else "  Duration: N/A")
        print(f"  Average solve time: {np.mean(data['solve_times']):.4f}s" if 'solve_times' in data else "  Average solve time: N/A")
        print()
        ...
        if 'controller_config' in data:
            ...
            print(f"  Position weight: {data['controller_config'].get('position_weight', 'N/A')}")
            print(f"  Quaternion weight: {data['controller_config'].get('quaternion_weight', 'N/A')}")
            print()

        if 'simulation_config' in data:
            print("SIMULATION CONFIG:")
            print(f"  Simulation dt: {data['simulation_config'].get('dt', 'N/A')}s")
            print(f"  Control dt: {data['simulation_config'].get('dt_control', 'N/A')}s")
