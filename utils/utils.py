...
def calc_quat_error(q, q_ref):
    ...
    norm_sq_q_ref = ca.dot(q_ref, q_ref)
...
def GTMRP_props(
    ...
    p = [
        l * axis_rot("z", i * (np.pi / (n / 2))) @ np.array([1, 0, 0]) + np.array(com)
        for i in range(n)
    ]
...
def GTMRP_matrix(
    R: list, p: list, c_f: list, c_t: list, sign: int = 1
    ...
    G_t = np.column_stack([
        np.cross(p[i], Riz[i]) + c_t[i] / c_f[i] * sign * (-1) ** i * Riz[i] for i in r
    ])
