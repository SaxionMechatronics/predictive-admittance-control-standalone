...
class ResultsVisualizer:
    ...
    def __init__(self, results: dict):
        ...
        if 'impedance_position' in results:
            self.impedance_position = results['impedance_position']
        else:
            self.impedance_position = np.zeros((len(self.times), 3))
        if 'impedance_velocity' in results:
            self.impedance_velocity = results['impedance_velocity']
        else:
            self.impedance_velocity = np.zeros((len(self.times), 3))
        
        # Extract contact forces if available
        if 'contact_forces' in results:
            self.contact_forces = results['contact_forces']
        else:
            self.contact_forces = np.zeros((len(self.times), 3))
        ...
        elif 'ref_pos_x' in results and 'ref_pos_y' in results and 'ref_pos_z' in results:
            self.ref_positions = np.column_stack([
                results['ref_pos_x'], results['ref_pos_y'], results['ref_pos_z']
            ])
        ...
        if 'ref_quaternions' in results:
            ...
            self.ref_quaternions = np.column_stack([
                results['ref_quat_w'], results['ref_quat_x'], 
                results['ref_quat_y'], results['ref_quat_z']
            ])
    ...
    def plot_all(self, save_path: Optional[str] = None):
        ...
        ax1.plot(self.positions[:, 0], self.positions[:, 1], self.positions[:, 2], 
                'b-', linewidth=2)
        ax1.scatter(self.positions[0, 0], self.positions[0, 1], self.positions[0, 2],
                   c='g', marker='o', s=100, label='Start')
        ax1.scatter(self.positions[-1, 0], self.positions[-1, 1], self.positions[-1, 2],
        ...
        if self.ref_positions is not None:
            ax2.plot(self.times, self.ref_positions[:, 0], 'r--', alpha=0.7, label='x ref')
            
        ax2.plot(self.times, self.positions[:, 1], 'g-', label='y')
        if self.ref_positions is not None:
            ax2.plot(self.times, self.ref_positions[:, 1], 'g--', alpha=0.7, label='y ref')
            
        ax2.plot(self.times, self.positions[:, 2], 'b-', label='z')
        if self.ref_positions is not None:
            ax2.plot(self.times, self.ref_positions[:, 2], 'b--', alpha=0.7, label='z ref')
        ...
        if self.ref_euler_angles is not None:
            ax4.plot(self.times, self.ref_euler_angles[:, 0], 'r--', alpha=0.7, label='Roll ref')
            
        ax4.plot(self.times, self.euler_angles[:, 1], 'g-', label='Pitch')
        if self.ref_euler_angles is not None:
            ax4.plot(self.times, self.ref_euler_angles[:, 1], 'g--', alpha=0.7, label='Pitch ref')
            
        ax4.plot(self.times, self.euler_angles[:, 2], 'b-', label='Yaw')
        if self.ref_euler_angles is not None:
            ax4.plot(self.times, self.ref_euler_angles[:, 2], 'b--', alpha=0.7, label='Yaw ref')
        ...
        ax12.plot(self.times, solve_times_ms, 'b-', alpha=0.6, linewidth=1, label='Solve time')
        ax12.axhline(y=np.mean(solve_times_ms), color='r', linestyle='--', 
                   linewidth=2, label=f'Mean: {np.mean(solve_times_ms):.2f} ms')
        ax12.axhline(y=np.max(solve_times_ms), color='orange', linestyle=':', 
                   linewidth=1.5, label=f'Max: {np.max(solve_times_ms):.2f} ms')
    ...
    def animate_3d(self, frame_skip: int = 10, save_path: Optional[str] = None):
        """
        ...
        def animate(frame_idx):
            ...
            return line_x, line_y, line_z, trajectory_line, drone_center, time_text
